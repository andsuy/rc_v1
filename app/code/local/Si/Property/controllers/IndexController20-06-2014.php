...
class Si_Property_IndexController extends Mage_Core_Controller_Front_Action{
    ...
    public function reviewstatusAction() {
     ...
     public function recommendedpriceAction() {
     	...
     	if($ptype == 5) { $dtype = 'Shared room'; }
     	elseif($ptype == 6) { $dtype = 'Private room'; }
     	else { $dtype = 'Entire home/apt'; }
    	...
    	if(!empty($result)) {
    		...
    		$query = "SELECT AvgDailyPrice FROM RcAvgRoomPrice where city LIKE '%" . $rcity . "%' AND roomtype LIKE '%". $dtype ."%'";
    		//echo $query . '<br>';
    		$result = $readConnection->fetchOne($query);
    		if(!empty($result)) {
	    		$resultPrice = $result;
