...
class Apptha_Sociallogin_IndexController extends Mage_Core_Controller_Front_Action {
	 ...
	 * @return string
	 */
	public function customerAction($firstname, $lastname, $email, $provider, $userId = '', $about_me = '', $user_friends = array()) {
		 ...
		 */
		if ($customer->getId()) {
			if($customer->getFacebookUserid() == '') {
				$customer->setFacebookUserid($userId);
				$customer->save();
			}
			if($about_me != '') {
				$customerProfile = Mage::getModel('customerextend/customerinfo')->load($customer->getId(), 'customer_id');
				//echo 'PC ID = ' . $customerProfile->getPcId(); exit;
				$customer_prof = Mage::getModel('customerextend/customerinfo');
				if($customerProfile->getPcId()) {
					$pcinfoData = array(
									'host_details' => $about_me,
									'update_at' => now()
								);
					$customer_prof->setId($customerProfile->getPcId())->setData($pcinfoData);
					$customer_prof->save();
				} else {
					$pcinfoData = array(
					...
					$customer_prof->setId(null)->setData($pcinfoData);
					$customer_prof->save();
				}
			}
			
			// customer friends list data from facebook
			if(count($user_friends['data']) > 0) {
				//echo count($user_friends['data']);
				//echo '<pre>'; print_r($user_friends); echo '</pre>'; exit;
				
				$resource = Mage::getSingleton('core/resource');
				$writeConnection = $resource->getConnection('core_write');
				$readConnection = $resource->getConnection('core_read');
				$tableName = $resource->getTableName('social_friendlist');
				
				$customerId = $customer->getId();
				foreach($user_friends['data'] as $_friend) {
					$friendFbId = $_friend['id'];
					$friendName = $_friend['name'];
					
					$selQuery = "SELECT `friend_id` FROM $tableName WHERE `friend_fb_id` = '$friendFbId' AND `customer_id` = $customerId ;";
					
					$friend_id = $readConnection->fetchOne($selQuery);
					//echo $friend_id;
					if($friend_id) {
					$query = "INSERT INTO $tableName (friend_id, friend_fb_id, customer_id, friend_name, created_at, update_at)
								VALUES ($friend_id, '$friendFbId', $customerId, '$friendName', now(), now())
								ON DUPLICATE KEY UPDATE friend_name = '$friendName', update_at = now();";
					} else {
					$query = "INSERT INTO $tableName (friend_id, friend_fb_id, customer_id, friend_name, created_at, update_at)
								VALUES ('', '$friendFbId', $customerId, '$friendName', now(), now())
								ON DUPLICATE KEY UPDATE friend_name = '$friendName', update_at = now();";
					}
					//echo $query; exit;
	...
	public function fbloginAction() {
		...
		if ($user) {
			try {
				...
				/**
				 * Fetches the user info from facebook from the array $user_profile
				 */
				...
				//exit;
				/**
				 * If FB login failed throws the error message
				 */
				if ($email == '') {
					...
					/**
					 * Do the customer account action
					 */
					$this->customerAction($firstname, $lastname, $email, 'Facebook', $user, $about_me, $user_friends);
				}
	...
	public function yahoopostAction() {
		...
		/**
		 * Creates the customer by the use of OAUTH_CONSUMER_KEY, OAUTH_CONSUMER_SECRET, OAUTH_APP_ID
		 */
		...
		if ($hasSession == FALSE) {
			...
			if ($session) {
				 ...
				 *
				 * @return array $user - Yahoo authendicated User session
				 */
				$user = $session->getSessionedUser();

				/**
				 * Get the profile infor from current logged-in user
				 *
